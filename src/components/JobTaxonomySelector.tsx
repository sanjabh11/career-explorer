...
const calculateAPO = (item: any, category: string): number => {
  ...
  const categoryKey = category as keyof typeof apoCategoriesPercentages;
  for (const [key, value] of Object.entries(apoCategoriesPercentages[categoryKey])) {
  ...
  const averageCategoryAPO = Object.values(apoCategoriesPercentages[categoryKey]).reduce((a, b) => (a as number) + (b as number), 0) / Object.values(apoCategoriesPercentages[categoryKey]).length;
...
const getAverageAPO = (items: any[], category: string): number => {
...
const JobTaxonomySelector: React.FC = () => {
  ...
  const renderList = (title: string, items: any[], category: string) => {
  ...
  const renderAdditionalDetails = (details: OccupationDetails) => {
    return (
  ...
  const renderAutomationAnalysis = (occupation: OccupationDetails | null) => {
